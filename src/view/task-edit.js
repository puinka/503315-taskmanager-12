...
const createRepeatingDaystemlate = (repeating) => {
  return Object.entries(repeating).map(([day, repeat]) => `<input
...
const createTaskEditRepeatingTemplate = (repeating, isRepeating) => {
  let repetingDaysTemplate = `<button class="card__repeat-toggle" type="button">
  repeat:<span class="card__repeat-status">${isRepeating ? `yes` : `no`}</span>
</button>`;
  if (isRepeating) {
    repetingDaysTemplate += `<fieldset class="card__repeat-days">
    <div class="card__repeat-days-inner">
      ${createRepeatingDaystemlate(repeating)}
    </div>
  </fieldset>`;
  }
  return repetingDaysTemplate;
};
